...
SELECT ltrim('          문자열   ');	-- 외쪽 공백 제거
SELECT rtrim('          문자열   '); 	-- 오른쪽 공백 제거
SELECT trim('-'  from '---문자열---');	-- 특정 문자 제거

-- replace(원본문자열ㅡ 찾을문자열, 대체 문자열) : 다른 문자열로 대체
...
-- 5. 집계 함수 : count, sum, avg, min, nax ->Groupby에서 다룸

-- 6. 시스템 정보 함수

SELECT version();		-- MySql 서버 버전 반환ALTER
